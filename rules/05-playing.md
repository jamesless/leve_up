...
2. 程序判断是否有玩家手中该花色的牌能够"管上"甩牌中的任意牌型
3. 如果有玩家能管上：
   - 甩牌失败
...
**游戏界面提示**（甩牌失败时）：
- 显示："玩家X的[牌型]管上了你的[牌型]"
- 显示："请收回[需收回的牌]，只出[保留的牌型]"
- 例如："玩家B的黑桃对8管上了你的黑桃对7，请收回黑桃6、9、J、Q，只出黑桃对7"

**验证规则**：
- **先看牌型，再看点数**：三张 > 对子 > 单张（牌型优先级）
- 对子只能被对子管上，单张只能被单张管上
- 如果能管上任意一个牌型，甩牌就失败
- **程序自动处理**：无需玩家手动确认，系统自动判断并验证
...
**示例3：甩牌失败（多种牌型被管上，选最小牌型）**
```
甩牌者出：方片对10 + 方片单7 + 方片单Q + 方片单K
程序检查：
  - 玩家D手中有方片对J（能管上方片对10）
  - 玩家E手中有方片单8（能管上方片单7）
游戏提示："玩家E的方片8管上了你的方片7"（选择最小被管上的牌型）
收牌验证：甩牌者必须收回方片对10、单Q、单K
结果：甩牌者只出方片单7
```
...
### 场景 5.14：甩牌失败-多种牌型被管上选最小
- **前置条件**：玩家出方片对10 + 方片单7，玩家D有方片对J，玩家E有方片单8
- **测试步骤**：程序检查发现两种牌型都被管上
- **预期结果**：
  - 选择最小被管上的牌型（方片单7）
  - 游戏提示显示"玩家E的方片8管上了你的方片7"
  - 甩牌者收回方片对10，只出方片单7
