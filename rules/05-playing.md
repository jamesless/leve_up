...
3. 如果有玩家能管上，甩牌失败，程序自动让甩牌者只出最小的那个被管上的牌型
4. 如果没有任何玩家能管上，甩牌成功
...
程序检查：
  - 玩家B手中有黑桃对8（能管上黑桃对7）
结果：甩牌失败，程序自动让甩牌者只出黑桃对7（收回黑桃6、9、J、Q）
```
...
程序检查：
  - 玩家C手中有红桃单7（能管上红桃单6）
结果：甩牌失败，程序自动让甩牌者只出红桃单6（收回红桃对5、单8、单10）
```
...
程序检查：
  - 玩家D手中有方片对J（能管上方片对10）
  - 玩家E手中有方片单8（能管上方片单7）
结果：甩牌失败，程序自动让甩牌者只出方片单7（最小的被管上的牌型）
```
...
- **预期结果**：甩牌失败，程序自动让甩牌者只出黑桃对7

### 场景 5.12：甩牌失败-单张被管上
- **前置条件**：玩家出红桃对5 + 红桃单6，其他玩家有红桃单7
- **测试步骤**：程序自动检查发现红桃单7能管上
- **预期结果**：甩牌失败，程序自动让甩牌者只出红桃单6
