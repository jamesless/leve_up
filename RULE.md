...
详细规则见 [rules/03-bidding.md](rules/03-bidding.md)。

### 3.1 亮庄

- 玩家使用**级牌**亮庄，用哪门花色的级牌亮庄，哪门就是主牌花色
- 亮庄类型：单张 / 对子 / 三张（三张封顶无法被反）

### 3.2 反庄

- 反庄者所亮张数只需 **多于** 临时庄家所亮张数即可（封顶 3 张）
- 用临时庄家的级牌反庄：庄家不变，只变主花色（若反庄者级牌也相同则庄家转移）
- 用自己的级牌反庄：玩家变为临时庄家

### 3.3 翻底牌定庄

- 触发条件：发牌完成 + 初始/追加倒计时归零 + 全程无人亮庄
- 7 张底牌从左到右每 3 秒翻一张
- 翻到的牌点数等于场上某玩家等级时停止，由该玩家当庄；多人匹配按逆时针距首发人最近者当庄
- 7 张全翻完仍无匹配 → 首发人当庄，主花色取"从第七张往前找第一张非王牌"的花色
